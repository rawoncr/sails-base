...
module.exports = function ToHandleNewConnection(app){
  ...
  return function onConnect (socket){
    ...
    // Implementation of the method afterConnect, by Ramon Caraballo
    if (app.config.sockets.afterConnect) {
      app.config.sockets.afterConnect(socket);
    }
    ...
    (function runOnConnectListener(){
      ...
      loadSessionFromSocket(socket.handshake, app, function finishedLoading(err, session, sessionId){
        ...
        // Then run lifecycle callback, if this isn't the admin socket
        if (!socket.handshake.query._isSailsAdminSocket_) {
    ...
    socket.on('disconnect', function onSocketDisconnect(){
      ...
      // (default: do nothing.  Also do nothing when the admin socket disconnects.)
      if (!app.config.sockets.afterDisconnect || socket.handshake.query._isSailsAdminSocket_) return;
